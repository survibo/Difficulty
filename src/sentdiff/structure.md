...
```
structure = 0.27×length + 0.20×embedding + 0.16×predicate
...
같은 lemma를 가진 내용어가 여러 번 등장할 때 반복 처리 부담을 반영한다.
표면형이 달라도 lemma가 같으면 하나의 반복 묶음으로 계산한다.

```
lemma별로 등장 횟수 count 수집
...
- `difficulty`: lemma의 lexical lookup 난도값. 반복 계산에서는 `max(0.10, min(0.5, difficulty / 1.5))`로 보정한다.
- `polysemy`: 같은 lemma에 속한 표면형들의 Kiwi analyze(top_n=5) 결과 중 가장 큰 서로 다른 품사 태그 가짓수
