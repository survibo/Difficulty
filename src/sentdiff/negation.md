...
|------|------|-----|
| MAG | 안, 못 | `안(부사)` |
| VX, VV | 않/못하/말로 시작하는 어간 | `않다`, `못하다`, `말다` |
| VA | 없 | `없다` |
| VCN | 아니 | `아니다` |
...
```
local_score = min(1.0, max(0, (local_max - 1) / 2))
```

같은 negation unit 내에 부정이 여러 개 있으면 중첩 부정으로 간주.
예: `안 먹지 않았다` → 한 unit에 부정 2개 → (2-1)/2 = 0.5

### 2. construction — 조건절 분할 부정

```
construction_score = 1.0  (조건절 link + 동일 hard segment 내 두 번째 부정)
                     0.0  (그 외)
```

"가지 않으면 안 된다" 식의 조건절-귀결 이중 부정. 동일 hard segment 내에서
조건절 link를 건너 두 번째 부정이 나타나면 1.0.
