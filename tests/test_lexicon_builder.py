...
class DerivationBaseSelectionTests(unittest.TestCase):
    def test_base_selection_prefers_nominal_pos(self) -> None:
        rows = pd.DataFrame([
            {
                **_row("기준", difficulty=0.1, pos_norm="동사"),
                "source": "vocab_40k_only",
            },
            {
                **_row("기준", difficulty=0.2, pos_norm="명사"),
                "source": "vocab_40k_only",
            },
        ])
    ...
    def test_base_selection_treats_compound_pos_as_preferred_nominal(self) -> None:
        rows = pd.DataFrame([
            _row("기준", difficulty=0.1, pos_norm="동사"),
            _row("기준", difficulty=0.2, pos_norm="부사/명사"),
        ])
        chosen = _choose_derivation_base_row(rows)
        self.assertIsNotNone(chosen)
